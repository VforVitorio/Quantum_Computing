"""
PROGRAMA: Demostración de la puerta cuántica RX (Rotación sobre eje X)
==========================================
Este programa demuestra el efecto de la puerta RX sobre un qubit inicializado en |0⟩.
...
# Importar las puertas cuánticas: I (Identidad) y RX (rotación sobre X)
from pyquil.gates import I,RX
...
# RX(π/2)|0⟩ = cos(π/4)|0⟩ - i·sin(π/4)|1⟩ ≈ 0.707|0⟩ - 0.707i|1⟩
# Esta puerta crea una superposición cuántica con componentes imaginarias
prog.inst(RX(math.pi/2,0))

# Ejecutar el programa completo (I + RX) y obtener la función de onda
result = qvm.wavefunction(prog)
# Imprimir la función de onda después de aplicar RX(π/2, 0)
# Resultado esperado: (0.707+0j)|0⟩ + (0-0.707j)|1⟩
print(result)
...
(0.707+0j)|0⟩ + (0-0.707j)|1⟩  ó  aproximadamente (0.7071+0j)|0⟩ + (0-0.7071j)|1⟩

EXPLICACIÓN: La puerta RX(π/2) transforma el estado |0⟩ en una superposición cuántica.
A diferencia de las puertas de fase (PHASE, S, T), RX cambia las AMPLITUDES de |0⟩ y |1⟩.
El resultado es cos(π/4)|0⟩ - i·sin(π/4)|1⟩ ≈ 0.707|0⟩ - 0.707i|1⟩.
Las probabilidades de medir |0⟩ o |1⟩ son iguales: |0.707|² = 0.5 = 50%.
La componente |1⟩ tiene un factor -i (fase negativa imaginaria).
"""
